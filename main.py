...
def anonymize_sql_query(query):
    ...
    query = re.sub(r'\bWHERE\s+([\w\.]+)', lambda m: f'WHERE {replace_column(m)}', query, count=1, flags=re.IGNORECASE)
    ...
    query = re.sub(r'\bORDER\s+BY\s+([\w\.]+)', lambda m: f'ORDER BY {replace_column(m)}', query, count=1, flags=re.IGNORECASE)
